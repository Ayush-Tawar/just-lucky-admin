...
export default function AboutUs() {
  const { apis } = useAuth();
  const { onUpdate, isSubmitting, isLoading, data } = apis.aboutusApi;
...
const Form = ({ onSubmit, defaultValues, isSubmitting }) => {
  const schema = Yup.object().shape({
    ...
    other: Yup.object().shape({
      ...
      dinings: Yup.string().required("required"),
      events: Yup.string().required("required"),
      cheers: Yup.string().required("required"),
    }),
  });
  ...
  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <RHFEditor name={`other.description`} label="Description" />
        </Grid>
        {fields.map((io, index) => {
          return (
